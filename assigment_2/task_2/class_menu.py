...
class Menu:
    ...
    def press1(self):
        ...
        def printer(Df):
            """
            Зrints summary for last 7 days
            :param Df: dataframe for the whole time
            """
        ...
        # Count how many days were between beginning and current session
        df2 = self.df.copy()
        df2["Session beginning"] = pd.to_datetime(df2["Session beginning"])
        df2 = self.df[
            df2["Session beginning"] >= (df2["Session beginning"] - timedelta(days=7))
        ]
        printer(df2)
        return
    ...
    def press2(self):
        ...
        if user_id not in self.df["User"].tolist():
            ...
            try:
                # set default dates
                if start_date == "":
                    start_date = user_data.sort_values(by="Session beginning").iloc[0][
                        "Session beginning"
                    ]
                    end_date = user_data.sort_values(by="Session beginning").iloc[-1][
                        "Session beginning"
                    ]
                else:
            ...
            if len(cur_user_data) != 0:
                ...
                print(
                    ...
                    cur_user_data.sort_values(by="Session beginning")
                    .iloc[0]["Session beginning"]
                    .date(),
                )
                print(
                    "\tDate of most recent session :",
                    cur_user_data.sort_values(by="Session beginning")
                    .iloc[-1]["Session beginning"]
                    .date(),
                )
                print(
                    "\tAverage time spent per session :"
                    + str(int(cur_user_data["Spended_time"].mean()))
                    + " min"
                )
                print(
                ...
                print(
                    "\tAvg dropped frames :",
                    int(cur_user_data["Dropped_frames"].mean()),
                )
                print("\tAvg bitrate :", int(cur_user_data["Bitrate"].mean()))
                print(
                    "\tTotal num of bad sessions: ",
                    sum(cur_user_data["Stream_quality"] == 1),
                )
                print(
                    "\tEstimated next session time :",
                    str(int(cur_user_data["Spended_time"].mean())) + " min",
                )
    ...
    def press3(self):
        """
        Predict user's next session time
        """
        print("\nEnter user id:")
        user_id = str(input())
        # check that user exists
    ...
    def press5(self):
        """
        Get top5 users with longest spended time
        """
